...
The brief offered three directions for the one improvement: make the import easier to trust, make the editor easier for a non-technical inspector, or handle a difficult case well. I chose trust. An inspector switching software has one question, "did my template survive", and the honest answer is a check, not a claim. The editor came second: the version a non-technical inspector wants is a rich-text editor, and that conflicts with storing HTML exactly as exported, so the editor stayed plain and the effort went into proof. Difficult cases got the eight fixtures and the parser hardening as a floor, not as the improvement. The Hive trial confirmed the choice: Hive's own importer is faithful on content and tells the inspector nothing about it.
